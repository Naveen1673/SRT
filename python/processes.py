...
def list_message(message, user_id, bot):
    ...
    if len(text_message) > 4000:
        splited_text_message = telebot.util.split_string(text_message,4000)
...
def kill(message, user_id, bot):
    bot.send_message(user_id,'Send me a process name to kill (case sensitive)')
    ...
    for process in processes_list:
        if process != []:
            if process[0] == pid_name:
                ...
                p_temp.terminate()
    bot.send_message(user_id,'Process %s successfully killed' %message.text)
