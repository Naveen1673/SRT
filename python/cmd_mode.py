...
def cmd_working_flow(message, user_id, bot):
    if message.text == '🔼 Back to Main':
        ...
        if outputText:  # Output checking
            oneMessage = outputText.encode('cp1251').decode('cp866')[:4088]
            if len(oneMessage) > 4000:
                splited_text_message = telebot.util.smart_split(oneMessage, 4000)
                for oneMessage in splited_text_message: # Splitting long messages
                    bot.send_message(message.from_user.id, oneMessage)
